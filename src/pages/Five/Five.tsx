...
export default function Five() {
  ...
  useEffect(() => {
    ...
    if (!video) return;
    if (!content?.video_placeholder_url) return;
    function loadVideo() {
      if (!video) return;
  ...
  }, [videoExists, content?.video_placeholder_url]);
    ...
    <>
      <div className="relative flex flex-col items-center w-full h-full z-[500]">
        <div className="flex flex-col gap-[3%] w-full h-full items-center mt-[10%]">
          ...
          {videoExists && (
            <div className="relative min-w-[86.2vw] min-h-[22vh] max-h-[22vh] aspect-[16/9]">
              <video
                preload="metadata"
                poster={`${URL}/data/${title}/${content?.video_placeholder_url}`}
          ...
          <div
            ...
            className="custom-scroll text-left w-[82vw] overflow-y-auto overflow-x-hidden h-[20vw] px-[2vw] text-[2vw] flex grow-[0.6] whitespace-pre-wrap"
          ></div>
