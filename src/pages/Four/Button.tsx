...
export default function Button({children, to, iconDirection, width = 45, py = 3}: {children: React.ReactNode, to: string, iconDirection: "left" | "right", width?: number, py?: number}) {
    return (
        <Link to={to}
        className={`w-full min-w-[${width}vw] max-w-[${width}vw] text-center cursor-pointer uppercase rounded-full py-[${py}vw] px-[7vw] text-white  overflow-hidden relative bg-gradient-to-r from-[#05CCA3] via-[#46AFE1] to-[#8085F0] text-[2vw] flex items-center justify-center`}>
